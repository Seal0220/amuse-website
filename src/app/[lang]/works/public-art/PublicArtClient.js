...
export default function PublicArtClient({ lang, timeline }) {
  ...
  const timelineYears = useMemo(
    () => safeTimeline.map((entry) => normalizeYearLabel(entry.year)),
    [safeTimeline],
  );

  const years = useMemo(() => {
    const base = timelineYears.length > 0 ? timelineYears : ['N/A'];
    return [...base, '__CENTER__'];
  }, [timelineYears]);

  const worksByRing = useMemo(
    () => [...safeTimeline.map((entry) => entry.works ?? []), []],
    [safeTimeline],
  );
  ...
  const diamAt = useCallback((i) => maxDiam - i * base, [maxDiam, base]);

  const isCenter = useCallback((y) => y === '__CENTER__', []);
  const isGap = useCallback((y) => y === '__GAP__', []);
  const isYear = useCallback((y) => !isCenter(y) && !isGap(y), [isCenter, isGap]);
  ...
  }, [animator, pageProgress, years, diamAt, isYear]);
