...
export default function useAnimator(animatorRef) {
    ...
    const seek = (targetProgress, { smooth = true, duration = 800 } = {}) => {
        if (!metrics.height || !window) return;
        const targetY = metrics.top + targetProgress * window.outerHeight;
        ...
        const nowProgress = (window.scrollY - (metrics.top || 0)) / window.outerHeight;
        animations.current.forEach((anim) => anim.apply(nowProgress));
    };
    ...
    return { useAnimation, setUpdateInterval, seek, start, stop, debug, metrics };
}
